...
from model.Tirano_multi import Tirano
from utils import *
from eval import *
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Tirano')
    ...
    parser.add_argument('--name', type=str, default='Tirano', help='name of the run')
    ...
    try:
        ...
        print(f"[INFO] Loaded relation2alpha from {alpha_file}, size={len(relation2alpha)}")
        logger.info(f"Loaded relation2alpha from {alpha_file}, size={len(relation2alpha)}")
    except:
        print(f"[WARN] Failed to load {alpha_file}, fallback = empty dict.")
        logger.info(f"Failed to load {alpha_file}, fallback to empty dict.")
        relation2alpha = {}
    ...
    # initialize model
    model = Tirano(nf, args.embsize, num_entities, num_relations, logger, decoder=args.score_func, steps=args.step,
                   device=device)
    model.to(device)
    ...
    if args.test == 0: # training
        ...
        for epoch in range(args.num_epoch):
            ...
            for batch in tqdm(train_data_loader):
                ...
                #loss= model.loss(score, batch.target_idx)
